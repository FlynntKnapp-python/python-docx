...
def create_document_table_by_cols(doc: Document, items: list, cols=None) -> Document:
    # Define the number of rows from the length of the items list and the number of
    # columns
    if len(items) % cols != 0:
...
doc_01 = create_document_table_by_cols(doc_00, items, cols=2)
